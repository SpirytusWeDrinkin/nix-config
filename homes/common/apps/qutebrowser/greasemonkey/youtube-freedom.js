...
(function () {
    ...
    selectors.forEach((sel) => {
      document.querySelectorAll(sel).forEach((dlg) => {
        const isAdblockWarning =
          ...
          /adblock|allow\s+ads|blocker|advertising|turn\s+off/i.test(
            dlg.textContent,
          ) ||
  ...
  function bypassAgeRestriction() {
    const ageDialog = document.querySelector(
      'ytd-enforcement-message-view-model, [class*="age-restriction"]',
    );
    ...
    if (player && player.paused && player.readyState === 0) {
      const isAgeBlocked = document.querySelector(
        'ytd-player .ytd-watch-flexy[ad-blocked], [class*="age-restricted"]',
      );
  ...
  function removeAdBanners() {
    ...
    selectors.forEach((sel) => {
      document.querySelectorAll(sel).forEach((el) => {
        if (
          /ad|advertisement|sponsored|promo/i.test(el.textContent) ||
